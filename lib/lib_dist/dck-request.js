...
const dispatchRequest = (config) => {
  ...
  config.url = baseURL + url + Object(_utils__WEBPACK_IMPORTED_MODULE_0__["paramsToSearch"])(params)
    ...
    uni.request({
      ...
      complete(res) {
        ...
        if (errMsg === 'request:ok') {
          ...
          if (Object(_utils__WEBPACK_IMPORTED_MODULE_0__["validateStatus"])(statusCode)) { // 请求成功
            resolve(response)
          } else { // 失败
            reject(Object(_utils__WEBPACK_IMPORTED_MODULE_0__["createError"])(`Request failed with status code ${statusCode}`, config, response))
          }
        } else if (errMsg === 'request:fail timeout') { // 请求超时
          reject(Object(_utils__WEBPACK_IMPORTED_MODULE_0__["createError"])(`timeout of ${config.timeout || 30000} ms exceeded`, config, null))
        } else { // 接口调用失败
          reject(Object(_utils__WEBPACK_IMPORTED_MODULE_0__["createError"])(`request:fail`, config, null))
        }
      },
    })
  })
